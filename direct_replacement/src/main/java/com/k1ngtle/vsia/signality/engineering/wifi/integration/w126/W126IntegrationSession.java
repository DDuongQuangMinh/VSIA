...
public final class W126IntegrationSession {
    ...
    private void mobilityIntegration(
        ...
        if (W119Mac.equals(sta1.wifiSelectedBssid(), ap2.wifiMacAddress())
            ...
            postRoamHttpStarted = sta1.startWifiRawHttpWorkflowWithExistingLease(InternetProviderRegistry.DEFAULT_WEB_DOMAIN, "/");
    ...
    private void postMobilityService(
        ...
        WifiRawIpWorkflowSnapshot workflow =
                sta1.wifiRawIpWorkflowSnapshot();

        if (!sta1BaselineIp.equals(
                sta1.wifiIpAddress()
        )) {
            fail(
                    W126Failure.IPV4_CHANGED_DURING_MOBILITY,
                    "Strict L3 continuity failed during post-roam service | expected="
                            + sta1BaselineIp
                            + " actual="
                            + sta1.wifiIpAddress()
                            + " workflow="
        ...
        if (workflow.state()
                == WifiRawIpWorkflowState.DHCP) {
            fail(
                    W126Failure.POST_MOBILITY_SERVICE_FAILED,
                    "Strict L3 continuity failed: post-roam service re-entered DHCP"
            );
            return;
        }

        if (!workflow.assignedIp()
                .isBlank()
                && !sta1BaselineIp.equals(
                workflow.assignedIp()
        )) {
            fail(
                    W126Failure.IPV4_CHANGED_DURING_MOBILITY,
                    "Strict L3 continuity failed: workflow lease "
                            + workflow.assignedIp()
        ...
        if (workflow.state()
            ...
            if (!workflow.detail()
                ...
                fail(
                        W126Failure.POST_MOBILITY_SERVICE_FAILED,
                        "Post-mobility HTTP did not return 200 | "
                                + workflow.detail()
                );
                return;
            }
            ...
            if (!sta1BaselineIp.equals(
                ...
                fail(
                        W126Failure.IPV4_CHANGED_DURING_MOBILITY,
                        "Strict L3 continuity failed at HTTP completion | workflow="
                                + workflow.assignedIp()
                                + " baseline="
                                + sta1BaselineIp
                );
            ...
            transition(
                    ...
                    "HTTP 200 passed after handoff using the existing IPv4 lease; no DHCP restart occurred"
            );
            return;
        }
        ...
        if (elapsedStage()
            ...
            fail(
                    W126Failure.POST_MOBILITY_SERVICE_FAILED,
                    "Post-mobility lease-reuse service timeout | state="
                            + workflow.state()
                            + " detail="
                            + workflow.detail()
            );
    ...
    private void scaleStability(
        ...
        if (elapsedStage() >= STABILITY_TICKS) {
            ...
            transition(
                    W126Stage.COMPLETE,
                    "PASS: Wi-Fi 1 / W1.26.4 — Wi-Fi Integration & Scale Validation | 2 WPA2 STAs | unique DHCP leases | concurrent HTTP 200 | ISP1 domain agreement | concurrent QoS | automatic AP handoff with strict IPv4 lease preservation | post-roam HTTP 200 without DHCP restart | both AP DS paths"
            );
        }
    }
