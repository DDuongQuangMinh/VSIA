...
public final class W128WebHostService {
    ...
    public static W128HttpResponse serveDirect(
            ServerLevel level,
            String rackIp,
            String requestedHost,
            String requestedPath,
    ...
    private static W128HttpResponse serveInternal(
            ServerLevel level,
            String rackIp,
            String requestedHost,
            String requestedPath,
        ...
        if (project == null) {
            return error(
                    404,
                    "Not Found",
                    "No W1.28 virtual host is published for "
                            + host
                            + "."
            );
        }

        W128WebBuildResult authority =
                web.validateAuthority(
                        level,
                        project,
                        nowMillis
                );
        ...
        if (project
            ...
            return error(
                    503,
                    "Service Unavailable",
                    "The website has no bound Server Rack IPv4 address."
            );
        }
        ...
        if (requireDns) {
            Optional<InternetDnsAnswer> dns =
            ...
            if (dns.isEmpty()
                ...
                return error(
                        421,
                        "Misdirected Request",
                        "ISP1 DNS no longer directs this host to this Server Rack."
                );
            }
        }
    ...
    public static String extractHost(
            OSINetworkPacket request
    ) {
        if (request == null) {
            return "";
        ...
        if (host != null
                && !host.isBlank()) {
            return W128WebRegistrySavedData
                    .normalizeHost(host);
        }
    ...
    public static byte[] responseWire(
            W128HttpResponse response
    ) {
        String body =
                response.body();
    ...
    private static W128HttpResponse error(
            int status,
            String reason,
            String body
    ) {
    ...
    private static boolean legacyHost(
            String host
    ) {
        String normalized =
                W128WebRegistrySavedData
