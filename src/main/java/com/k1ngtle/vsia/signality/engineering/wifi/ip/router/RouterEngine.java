...
public final class RouterEngine {
    ...
    public RouterInterface interfaceByName(String name){return interfaces.get(name);}
    ...
    public RouterInterface interfaceForLocalIp(String ip){
        return interfaces.values().stream()
                .filter(RouterInterface::enabled)
                .filter(i -> i.ipv4Address().equals(ip))
                .findFirst().orElse(null);
    }

    public RouterInterface interfaceForSourceNetwork(String ip){
        return interfaces.values().stream()
                .filter(RouterInterface::enabled)
                .filter(i -> i.contains(ip))
                .max(Comparator.comparingInt(RouterInterface::prefixLength))
                .orElse(null);
    }
    ...
    public RouterForwardDecision evaluate(String ingress, RouterPacket packet){
        ...
        RouterInterface local = interfaceForLocalIp(packet.destinationIp());
        if(local!=null)
            return decision(RouterForwardAction.LOCAL_DELIVERY,ingress,local.name(),packet,"","",packet.ttl(),-1,-1,0,0,"LOCAL "+local.name());

        if(packet.ttl()<=1)
            return decision(RouterForwardAction.ICMP_TIME_EXCEEDED,ingress,"",packet,"","",0,-1,-1,11,0,"TTL expired");

        RouterRoute best = routes().stream()
                .filter(r -> r.matches(packet.destinationIp()))
                .min(Comparator.comparingInt(RouterRoute::prefixLength).reversed()
                        .thenComparingInt(RouterRoute::metric))
                .orElse(null);
    ...
    private static RouterForwardDecision drop(String d){
        return new RouterForwardDecision(RouterForwardAction.DROP,"","","","","","",0,0,"",-1,Integer.MAX_VALUE,0,0,d);
    }
}
