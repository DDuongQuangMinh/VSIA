...
public final class W130CommandRegistry {
    private static final List<Command> COMMANDS = List.of(
            ...
            new Command("selftest.w130", "Developer: W1.30 Self Test", "Run complete W1.30 self-test", "w130 selftest"),
    ...
    public static List<Command> search(String query, int limit) {
        ...
        if (normalized.isEmpty()) {
            return COMMANDS.stream().limit(cap).toList();
        }

        List<Scored> scored = new ArrayList<>();
        for (Command command : COMMANDS) {
            String haystack = (command.label() + " " + command.description() + " " + command.command())
                    .toLowerCase(Locale.ROOT);
            int score = score(haystack, normalized);
            if (score >= 0) {
                scored.add(new Scored(command, score));
            }
        }

        return scored.stream()
                .sorted(Comparator.comparingInt(Scored::score)
                        .thenComparing(value -> value.command().label()))
                .limit(cap)
                .map(Scored::command)
                .toList();
    }
    ...
    private static int score(String haystack, String needle) {
        ...
        int direct = haystack.indexOf(needle);
        if (direct >= 0) {
            return 10 + direct;
        }
    ...
    private record Scored(Command command, int score) {
    }
}
